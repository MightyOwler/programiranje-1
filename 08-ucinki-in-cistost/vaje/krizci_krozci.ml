...
(* v polju lahko je igralec ali pa ga ni, torej ločimo na Some in None. Če dodamo ta option, moramo vedno kasneje
   pisati Some _ oz None*)
...
let winner_of_list triples : player option = 
  let folder acc triple = match acc with
  | Some p -> acc
...
let initial_state = OnTurn {player = X; grid = empty_grid}  (* in dodaj anotacijo [: state] *) 
...
let show_field = function
  | None -> " "
  | Some player -> show_player player
