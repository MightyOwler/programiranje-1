...
(* Ideja: definiramo zunanjo funckijo, notri pa definiramo neko pomožno funkcijo, ki pa je rekurzivna*)
(* OGRODJE ZA REPNO REKURZIVNO FUNKCIJO
let reverse lst = 
...
let range n = 
  let rec range_aux acc k = match k with
  | 0 -> 0 :: acc
  | k -> range_aux acc (k-1) @ [k]
  in range_aux [] n
...
let map_tlrec f list = 
  let rec map_tlrec_aux acc f list = match list with
  | [] -> acc
  | x :: xs -> map_tlrec_aux ((f x) :: acc) f xs
in map_tlrec_aux [] f (reverse list)
...
let mapi f list = 
  let rec mapi_aux i list = match list with
  | [] -> []
  | x :: xs -> (f x i) :: (mapi_aux (i + 1) xs)
in mapi_aux 0 list
...
| ([], []) -> []
| ([], _) -> failwith "en seznam je daljsi od drugega"
| (_, []) -> failwith "en seznam je daljsi od drugega"
...
let unzip list = 
  let rec unzip_aux acc1 acc2 list = match list with
  | [] -> (acc1, acc2)
  | (prvi, drugi) :: xs -> unzip_aux (prvi :: acc1) (drugi :: acc2) xs
in unzip_aux [] [] (reverse list)
...
let rec loop pogoj f = function
| x -> if (pogoj x) then loop pogoj f (f x) else x
...
let apply_sequence f x n = 
  let rec apply_sequence_aux acc x k = 
    if k <= 0 then 
      reverse acc 
    else 
      apply_sequence_aux ((f x) :: acc) (f x) (k - 1)
  in 
  apply_sequence_aux [x] x n
...
let filter f list = 
  let rec filter_aux acc f list = match list with
  | [] -> reverse acc
  | x :: xs -> if f x = true then filter_aux (x :: acc) f xs else filter_aux acc f xs
...
(* včasih se da napisati funkcijo tudi brez pomožne notranje *)
