...
def find_words(niz, podniz):
    return set(re.findall(r"\b\w*" + podniz + r"\w*\b", niz))
...
def find_prefix(niz, podniz):
    return set(re.findall(r"\b" + podniz + r"\w*\b", niz))
...
def double_letters(niz):
    return set(re.findall(r"(\b\w*(\w)\2\w*\b)", niz))

print(double_letters("success booing bubble"))